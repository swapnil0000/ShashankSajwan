...
import { Container, Typography, Button, Box, useTheme, useMediaQuery, Divider } from '@mui/material';
...
const CourseProvide = () => {
  ...
  const isSmallMobile = useMediaQuery(down("sm"));
  ...
  return (
    <Container sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '100%', marginTop: '50px' }}>
      <Typography sx={{ fontWeight: '700', fontSize: isSmallMobile ? "20px" : "40px" }} align="center">COURSES PROVIDED</Typography>
      <Box sx={{ width: '90%', height: 'auto', marginTop: '15px', boxShadow: 'rgba(100, 50, 93, 0.25) 10px 10px 12px 10px, rgba(0, 0, 0, 0.3) 0px 3px 7px -3px', borderRadius: '20px' }}>
        <Box sx={{ display: 'flex', flexDirection: isMobileView ? "column" : "row", justifyContent: 'space-between' }}>
          ...
          <Box>
            ...
            <Typography sx={{ margin: '8px', marginTop: '20px' }} variant='h4'>In this program, you will get Full-Fledged Preparation including with proper mentorship and guidance with all the materials.</Typography>
            <Box sx={{ display: 'flex', flexDirection: isSmallMobile ? "column" : "row", justifyContent: 'space-between' }}>
              <Box sx={{ margin: '10px', display: 'flex', flexDirection: 'column', fontSize: '15px' }}>
                {courseData.left.map(ele => (
                  <Typography key={ele.id} sx={{ display: 'flex', margin: '10px', gap: '5px', fontSize: isMobileView ? "15px" : '20px', fontSize: '17px' }}>{ele.icon}{ele.val}</Typography>
                ))}
              </Box>
              <Box sx={{ margin: '10px', display: 'flex', flexDirection: 'column' }}>
                {courseData.right.map(ele => (
                  <Typography key={ele.id} sx={{ display: 'flex', margin: '10px', gap: '5px', fontSize: isMobileView ? "15px" : '20px', fontSize: '17px' }}>{ele.icon}{ele.val}</Typography>
                ))}
              </Box>
            </Box>
